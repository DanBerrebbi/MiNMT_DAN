...
import torch
import math
from transformer.Model import prepare_source, prepare_target, prepare_prefix, prepare_prefix_1
  ...
  def translate(self, testset, output):
    ...
    with torch.no_grad():
      ...
      for pos, batch_idxs in testset:
        ...
        if self.model_type == 's_sc_s_scc':
          src, self.msk_src = prepare_source(batch_idxs[0], self.src_voc.idx_pad, self.device) #src is [bs, ls] msk_src is [bs,1,ls]
          xsrc, msk_xsrc = prepare_source(batch_idxs[1], self.src_voc.idx_pad, self.device) #xsrc is [bs, lxs] msk_xsrc is [bs,1,lxs]
          xtgt, _, self.msk_xtgt_1 = prepare_target(batch_idxs[2], self.tgt_voc.idx_pad, self.device) #xtgt is [bs, lxt] msk_xtgt is [bs,1,lxt]
          _, self.msk_xtgt_2 = prepare_prefix_1(batch_idxs[2], self.tgt_voc.idx_pad, self.device) #xtgt is [bs, lxt] msk_xtgt is [bs,1,lxt]
          self.z_src, self.z_xtgt = self.model.encode(src, xsrc, xtgt, self.msk_src, msk_xsrc, self.msk_xtgt_1) #[bs,ls,ed] [bs,lxt,ed]
    ...
    while True:
      ...
      if lt == 2:
        ...
        if self.model_type == 's_sc_s_scc':
          ...
          self.msk_src = self.msk_src.repeat_interleave(repeats=self.K, dim=0) #[bs,1,ls] => [bs*K,1,ls]
          self.z_xtgt = self.z_xtgt.repeat_interleave(repeats=self.K, dim=0) #[bs,lxt,ed] => [bs*K,lxt,ed]
          self.msk_xtgt_2 = self.msk_xtgt_2.repeat_interleave(repeats=self.K, dim=0) #[bs,1,lxt] => [bs*K,1,lxt]
        elif self.model_type == 'sxs_sc':
          self.z_src = self.z_src.repeat_interleave(repeats=self.K, dim=0) #[bs,ls,ed] => [bs*K,ls,ed]
          self.msk_src = self.msk_src.repeat_interleave(repeats=self.K, dim=0) #[bs,1,ls] => [bs*K,1,ls]
          self.z_xtgt = self.z_xtgt.repeat_interleave(repeats=self.K, dim=0) #[bs,lxt,ed] => [bs*K,lxt,ed]
      ...
      if self.model_type == 's_sc_s_scc':
        y_next = self.model.decode(self.z_src, self.z_xtgt, hyps, self.msk_src, self.msk_xtgt_2, msk_tgt=msk_tgt)[:,-1,:] #[I,lt,Vt] => [I,Vt]
      elif self.model_type == 'sxs_sc':
        y_next = self.model.decode(self.z_src, self.z_xtgt, hyps, self.msk_src, self.msk_xtgt, msk_tgt=msk_tgt)[:,-1,:] #[I,lt,Vt] => [I,Vt]
      elif self.model_type == 'sxsc_sc':
        y_next = self.model.decode(self.z_src, self.z_xtgt, hyps, self.msk_src, self.msk_xtgt, msk_tgt=msk_tgt)[:,-1,:] #[I,lt,Vt] => [I,Vt]
