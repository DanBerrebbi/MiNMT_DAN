...
import time
import codecs
from transformer.Model import save_checkpoint, prepare_source, prepare_target, prepare_prefix_1
import sacrebleu
try:
  ...
  def learn(self, trainset, validset, device):
    ...
    while True: #repeat epochs
      ...
      for batch_pos, batch_idxs in trainset:
        ...
        elif self.model.type() == 's_sc_s_scc': 
          batch_src, batch_tgt, batch_xsrc, batch_xtgt = batch_idxs[0], batch_idxs[1], batch_idxs[2], batch_idxs[3]
          src, msk_src = prepare_source(batch_src, self.idx_pad, device)
          tgt, ref, msk_tgt = prepare_target(batch_tgt, self.idx_pad, device)
          xsrc, msk_xsrc = prepare_source(batch_xsrc, self.idx_pad, device)
          xtgt, xref, msk_xtgt_1 = prepare_target(batch_xtgt, self.idx_pad, device)
          _, msk_xtgt_2 = prepare_prefix_1(batch_xtgt, self.idx_pad, device)
          pred, pred_pre = self.model.forward(src, xsrc, xtgt, tgt, msk_src, msk_xsrc, msk_xtgt_1, msk_xtgt_2, msk_tgt) #no log_softmax is applied
        ...
        ntok_in_pre_batch = torch.sum(xref != self.idx_pad)
        ntok_in_step += ntok_in_batch

        if self.model.type() == "s_sc_s_scc":
          """ print("pred shape: ", pred.shape)
          print("ref shape: ", ref.shape)
          print("pred pre shape: ", pred_pre.shape)
          print("xref shape: ", xref.shape) """
          loss = 0.8 * self.criter(pred, ref) / ntok_in_batch / self.accum_n_batchs + 0.2 * self.criter(pred_pre, xref) / ntok_in_pre_batch / self.accum_n_batchs
        else:
    ...
    with torch.no_grad():
      ...
      for batch_pos, batch_idxs in validset:
        ...
        if self.model.type() == 's_sc':
          ...
          tgt, ref, msk_tgt = prepare_target(batch_tgt, self.idx_pad, device)
          xsrc, msk_xsrc = prepare_source(batch_xsrc, self.idx_pad, device)
          xtgt, _, msk_xtgt_1 = prepare_target(batch_xtgt, self.idx_pad, device)
          _, msk_xtgt_2 = prepare_prefix_1(batch_xtgt, self.idx_pad, device)
          pred, _ = self.model.forward(src, xsrc, xtgt, tgt, msk_src, msk_xsrc, msk_xtgt_1, msk_xtgt_2, msk_tgt) #no log_softmax is applied
